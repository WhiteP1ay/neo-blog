 ...
 */
export async function getHomeExplorerData(): Promise<ActionResult<HomeExplorerCategory[]>> {
  try {
 ...
 */
export async function uploadAdminImageFromForm(formData: FormData): Promise<ActionResult<{ url: string }>> {
  const gate = requireAdminSession(await getSession());
  if (!gate.ok) {
    return actionErr(gate.error);
  }
