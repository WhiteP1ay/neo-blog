  ...
  return (
    <div className={`flex items-center gap-3 justify-end `}>
      ...
      >
        {/* 背景圆圈 */}
        <span
          className={`absolute inset-0 rounded-full transition-colors ${checked ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700'}`}
        />

        {/* 滑块 */}
        <span
          className={`relative inline-block h-5 w-5 transform rounded-full transition-transform ${checked ? 'translate-x-6 bg-white' : 'translate-x-1 bg-gray-300'}`}
        >
          {/* 图标容器 */}
