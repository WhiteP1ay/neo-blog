...
import { applyAiPolishToPost } from '@/server/utils/post-ai-polish-apply';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const applyInput =
      preview.afterHtmlEn !== null
        ? { content: preview.afterHtml, contentEn: preview.afterHtmlEn }
        : { content: preview.afterHtml };

    const data = await applyAiPolishToPost(db, postId, post, applyInput);
